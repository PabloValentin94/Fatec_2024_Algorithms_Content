...
// Descrição: entendendo o conceito do algoritmo BFS (Breadth-First Search).
...
function BFS(grafo, inicio = 1)
{

    let fila = [], vertices_visitados = [];
    ...
    while(fila.length > 0)
    {
        ...
        console.log(`Fila Atual (BFS): ${fila.join(", ")}.`);

        console.log(`Vértices visitados (BFS): ${(vertices_visitados.join() !== "") ? vertices_visitados.join(", ") : "Nenhum"}.\n`);

        let vertice_analisado = fila.shift();

        vertices_visitados.push(vertice_analisado);

        for(let vertice_adjacente of grafo[vertice_analisado])
        {

            if(!vertices_visitados.includes(vertice_adjacente))
            {
    ...
    console.log("Fila Atual (BFS): vazia.");

    console.log(`Vértices visitados (BFS): ${vertices_visitados.join(", ")}.`);

}

// Testes (Como pesos não influenciam no BFS, não foram implementados para diminuir a complexidade do código.).
...
BFS(grafo.adjacencias, parseInt(Object.keys(grafo.adjacencias)[0], 10));
